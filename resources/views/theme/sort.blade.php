...
@section('footer.scripts')
    ...
    <script>
        ...
        function refresh_club_sort_data() {
            $.ajax({
                type: "GET",
                url: "{{ route('theme_sort_data') }}",

                success: function (responseData) {
                    $("#theme_sort_data_div").html('');
                    $("#theme_sort_data_div").html(responseData);
                    /**************************/
                    $('#sortable').sortable({
                        update: function (event, ui) {
                            var themeOrder = $(this).sortable('toArray').toString();
                            $.post("{{ route('theme_sort_update') }}", {themeOrder: themeOrder, _method: 'PUT', _token: '{{ csrf_token() }}'})
                        }
                    });
